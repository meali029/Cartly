...
const Checkout = () => {
  ...
  const handleOrder = async (e) => {
    ...
    const orderPayload = {
      userId: user._id,
      items: checkoutItems.map(({ product, quantity, size }) => ({
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex items-center justify-center">
        <div className="bg-white rounded-2xl shadow-lg p-8 text-center max-w-md">
          ...
          <p className="text-gray-600 mb-4">Add some items to continue with checkout</p>
        ...
        <div className="container mx-auto px-4 py-6">
          <div className="flex items-center gap-3">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
              loading ? 'bg-yellow-500 animate-pulse' : 'bg-green-600'
            }`}>
              {loading ? (
        ...
        <div className="grid lg:grid-cols-2 gap-8 max-w-6xl mx-auto">
          ...
          <div className="lg:order-2">
            <div className={`bg-white rounded-2xl shadow-lg p-6 sticky top-6 transition-all duration-300 ${
              loading ? 'opacity-75 ring-2 ring-green-200' : ''
            }`}>
              <h2 className={`text-xl font-bold text-gray-900 mb-6 ${loading ? 'animate-pulse' : ''}`}>
                {loading ? 'Processing Order...' : 'Order Summary'}
              </h2>
              
              <div className="space-y-4 mb-6">
                {checkoutItems.map(({ product, quantity, size }) => (
                  <div key={`${product._id}-${size || 'default'}`} className="flex items-center gap-4">
                    <img
                      src={product.image}
                      alt={product.title}
              ...
              <div className="border-t border-gray-200 pt-4 space-y-2">
                ...
                {getShippingCost() === 0 && (
                  <p className="text-xs text-green-600">Free shipping on orders above PKR 5,000</p>
                )}
                <div className="flex justify-between text-lg font-bold pt-2 border-t">
                  <span>Total</span>
                  <span className="text-green-600">PKR {getFinalTotal().toLocaleString()}</span>
                </div>
              </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <form onSubmit={handleOrder} className={`space-y-6 ${loading ? 'opacity-75 pointer-events-none' : ''}`}>
                ...
                <div className="space-y-4">
                  <h3 className="font-semibold text-gray-900 border-b border-gray-200 pb-2">Personal Information</h3>
                ...
                <div className="space-y-4">
                  <h3 className="font-semibold text-gray-900 border-b border-gray-200 pb-2">
                    Delivery Address <span className="text-green-600 text-sm">(Pakistan Only)</span>
                  </h3>
